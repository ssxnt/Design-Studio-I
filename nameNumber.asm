...
; Wait 'R2' seconds               ;
;---------------------------------;
WaitSeconds: ; logic based on this video (estimate) = https://www.youtube.com/watch?v=A4fhCxknPXE&ab_channel=ABCROB
    push AR0
    push AR1
L6: mov R1, #255
L5: mov R0, #255
L4: djnz R0, L4 
    djnz R1, L5
    djnz R2, L6 ; number of milliseconds to wait passed in R2
    pop AR1
    pop AR0
    ret
...
myprogram:
    ...
    mov a, #0x01 ; Clear the screen, googled 8051 documentation
    lcall WriteCommand
    mov R2, #100
    lcall WaitSeconds
    ...
    mov a, #0x08 ; blinking functionality
    lcall WriteCommand
    mov R2, #50
    lcall WaitSeconds
    mov a, #0x0C 
...
string_left:
	mov a, #0x18 ; strings move toward the left
	lcall WriteCommand
	lcall Waitmillisec
	sjmp string_left
