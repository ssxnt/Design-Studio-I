...
const char* unitPrefix(int exponent)
{
	switch (exponent) {
		case -3:
			return "K";
...
void main (void) 
{
	...
	while(1)
	{
		...
		// print to PuTTY
		printf("\rC = %.5f %sF, f = %.5lu Hz", cap, unitPrefix(c_exp), F);
		printf("\x1b[0K"); // ANSI: Clear from cursor to end of line.

		// print to LCD
		if (unitPrefix(c_exp) == "u") {
			sprintf(c_str, "%.5f %sF", cap, unitPrefix(c_exp));
			sprintf(f_str, "%.5lu Hz", F);
			LCDprint(c_str, 1, 1);
			LCDprint(f_str, 2, 1);
		}
		else if ((int) cap / 10 == 1) {
			sprintf(c_str, "%.5f %sF", cap-0.99, unitPrefix(c_exp));
			sprintf(f_str, "%.5lu Hz", F);
			LCDprint(c_str, 1, 1);
			LCDprint(f_str, 2, 1);
		}
		else {
			sprintf(c_str, "%.5f %sF", cap-0.37, unitPrefix(c_exp));
			sprintf(f_str, "%.5lu Hz", F);
			LCDprint(c_str, 1, 1);
			LCDprint(f_str, 2, 1);
		}	
