...
char _c51_external_startup (void)
{
	...
	// TH0 = 0x100-((SYSCLK/BAUDRATE)/(2L*12L));
	// TL0 = TH0;      // Init Timer0
...
void main (void) 
{
	float F = 0;
    float P = 0;
	char f_str[CHARS_PER_LINE];
	char a_str[CHARS_PER_LINE];
	float v[2];
	...
	while(1)
	{
		...
		// while (P1_5==0); // Wait for the signal to be one
		// TR0=1; // Start timing
		// while (P1_5==1); // Wait for the signal to be zero
		// TR0=0; // Stop timer 0
		...
		voltage1 = Volts_at_Pin(QFP32_MUX_P1_7) * 2.0;
		...
		voltage2 = Volts_at_Pin(QFP32_MUX_P1_6) * 2.0;

		TR0=0; // Stop timer 0
		phase_diff=TH0*256.0+TL0; // The 16-bit number [TH0-TL0]

		// print to PuTTY
		if(voltage1 != 0 && voltage2 != 0) {
			sprintf(a_str, "V1:%1.2f V2:%1.2f", voltage1, voltage2);
        	LCDprint(a_str, 1, 1);

			sprintf(f_str, "T: %3.2f ms", P);
        	LCDprint(f_str, 2, 1);

			printf("\rP: %3.3f ms, V1: %1.4f, V2: %1.4f, Phzr: %1.4f", P, voltage1, voltage2, phase_diff);
			// printf("\rP: %3.3lu ms, V1: %1.4f, V2: %1.4f, Phzr: %1.4f", period, voltage1, voltage2, phase_diff);
		}
	}	
}
